...
- **Anthropic**: Prompt caching is enabled via explicit `cache_control: {"type": "ephemeral"}` breakpoints in `internal/provider/anthropic.go`. The `system` field is sent as an array of content blocks (not a plain string — this is required to attach `cache_control`). Two breakpoints are used: one on the last tool definition (caches the tool block) and one on the last system block (caches tools + system prompt). Prefix order is tools → system → messages, so the system breakpoint covers everything through the system prompt. The provider already reads `cache_creation_input_tokens` and `cache_read_input_tokens` from the response and surfaces them in `TokenUsage` — these will now be populated. Minimum cacheable prefix is 1,024 tokens (Sonnet/Haiku) or 4,096 (Opus 4.5+). Cache is 5-minute TTL by default; each cache hit resets the TTL timer.
...
- **Key gotcha**: The system prompt currently includes the current date (`time.Now()`) and working directory in the cached prefix. The date changes every turn, which means the system prompt is NOT byte-for-byte identical across turns. This means the cache breakpoint on the system block will miss every turn (cache write but never a read) — the only reliable cache hit comes from the tool definitions breakpoint. To fully benefit from system prompt caching, the date and other per-turn dynamic content should be moved out of the system prompt into the message history (like Claude Code does with `<system-reminder>` tags).
