...
- **What**: A pure-Go sentence-embedding provider (`thenlper/gte-small`, 384-dim) runs in-process so agentic memory needs **no API key and no external service**. Selected as provider id `"local"` (the default in the settings UI). Chosen over `all-MiniLM-L6-v2` for higher MTEB score (~61.4 vs ~56.3) and no query/passage prefixing requirement.
