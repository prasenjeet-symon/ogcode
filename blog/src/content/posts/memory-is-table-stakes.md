...
description: "Grok Build just learned to remember your project. Good — every agent should. Here's how Ogcode remembers: markdown on your disk, one note per turn, recalled only when a turn actually needs it."
pubDate: 2026-09-20
...
*output* — file contents, command logs, API responses — never goes in. That's
the entire token economy of the system. The note remembers *that*
...
it. Sessions that aren't memorable units of work — sub-agents, background
indexing, the recall agent itself — are excluded, so memory never fills up
with the agent talking to itself.
...
plus the previous turn's final response for continuity — and everything older
stays home, on disk, already written down. What turn forty costs is shaped by
what turn forty *does*, not by the thirty-nine turns before it.
...
When the agent needs history, it calls a recall tool — scoped to the current
conversation, or, with `project_memory_recall`, to every conversation this
workspace has ever had. The tool runs a separate librarian agent that is
...
the model cannot ask its way into a wider scope than the turn granted. And
recall waits for any note still being written in the background, so what it
searches is complete up to the previous turn, every time.

Plan Mode drinks from the same well: the planning agent opens every session
by reading `.ogcode/archives/` — your locked plans — and your saved notes
before it asks you a single question.
...
Two more files ride along, and you may already have them. `AGENT.md` — or the
cross-tool `AGENTS.md`, which Ogcode reads the same way — holds how you want
the agent to behave. `MEMORY.md` holds durable facts, and Ogcode records them
there on its own initiative the moment they prove out: *tried X → got Y → do
Z instead*. Both are discovered by walking up from the working directory, so
...
## When one turn is the problem

Cross-session memory doesn't help when a single turn reads half your repo.
For that, Ogcode lets the agent compact its own context **mid-turn**: it
writes a summary of the work so far, and from the next step on, the request
is that summary plus what came after — while the session store keeps every
message, so nothing you see in the UI is lost. If a request still approaches
the context window, older history is summarized proactively and prior
summaries are folded in rather than dropped. The context limit stops being a
wall and becomes a budget.
...
- **A memory view in the UI.** Today the browser is your file manager. A
  first-class view — search, preview, prune — belongs in the web UI too.
...
- **A global scope.** Memory is per-project by design; preferences that are
  about *you* — how you like commits, reviews, explanations — deserve a home
  that follows you across repos.
- **Secret scrubbing.** Notes capture tool arguments verbatim. Keep secrets
  in your environment, not your prompts — good advice with any agent, and
  we'd rather enforce it mechanically than advise it.
