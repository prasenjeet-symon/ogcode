...
**Several changes to one file belong in one "edit" call.** Pass them as its "edits" array: they apply in order against each other's results, and all-or-nothing — if any anchor is missing or ambiguous, nothing is written and the error names which entry failed. That is one round trip and cannot half-apply a refactor. Separate same-file "edit" calls remain safe (the runtime serializes mutations to the same path, so each re-reads after the last applied) but they commit independently, and an overlapping anchor still fails cleanly — "old_string not found" when the winner consumed it, or an ambiguity error if the winner's replacement duplicated it — so re-issue that one after seeing the result. **Never batch a "write" with an "edit" to the same file** — order is unspecified, so the edit can anchor against content the write replaces. Verification batches freely — "check_syntax" on every file you touched belongs in a single block.`
	}
