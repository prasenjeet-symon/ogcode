...
- Every "edit" passes its changes in the "edits" array — one change is an array of one, and there is no other form. Several changes to ONE file belong in a single call: they apply all-or-nothing, so a refactor whose fourth change fails leaves the file untouched instead of partly rewritten. They apply in order and all-or-nothing, so a refactor whose fourth change fails leaves the file untouched instead of partly rewritten. Separate same-file "edit" calls remain safe — the runtime serializes mutations to the same path — but each commits on its own, so a later failure cannot undo an earlier success. Never batch a "write" with an "edit" to the same file — order is unspecified, so the edit can anchor against content the write replaces. Edits to different files batch freely.
