...
// worker, keeping the user's branch (the work survives on user/<slug> and can
// be merged later). The repo must already have a placement — assignment is
// what teaches the master where a repo lives.
...
// DeprovisionRepo retires a repo across the control plane: the placement
// worker removes every remaining user worktree (branches kept) and, when
// removeClone is true, deletes the clone itself. On success the placement is
// forgotten here and any of this repo's user sessions still routed to that
// worker are unrouted, so a stopped session does not resurrect a retired
// repo. Returns the worker's removal summary.
func (s *Server) DeprovisionRepo(ctx context.Context, repoURL string, removeClone bool) (string, error) {
	...
	for _, sid := range s.reg.SessionsForWorker(rec.WorkerID) {
