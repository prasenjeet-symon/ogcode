...
This release hardens deployment and first boot, adds **desktop
notifications** for finished loops and pending approvals, switches the UI
typeface to **Geist**, and ships a `bench/` directory of adapters that run
ogcode under third-party SWE harnesses.
...
None. Desktop notifications, the font switch and the deployment fixes need no
schema change or config migration; the `bench/` directory is optional tooling
and is not installed with the binary.
